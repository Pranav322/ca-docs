...
# import pdfplumber  # Disabled to prevent libpdfium segfaults
import pandas as pd
...
try:
    ...
    logger.warning(
        "Camelot not available - table extraction will use pdfplumber and tabula only"
    )
...
class OptimizedPDFProcessor:
    ...
    async def extract_text_and_tables_async(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"PDF processing completed in {processing_time:.2f}s - Found {len(final_results['text_chunks'])} text chunks and {len(final_results['tables'])} tables"
            )
    ...
    def _process_with_pdfplumber(self, pdf_path: str) -> Dict[str, Any]:
        """Extract tables using pdfplumber - disabled"""
        return {"tables": []}
        # try:
        #     tables = []
        #
        ...
        #                         table_data = self._process_table_data(
        #                             table, page_num + 1, table_idx, "pdfplumber"
        #                         )
        #                         if table_data:
        #                             tables.append(table_data)
        #                     except Exception as e:
        #                         logger.warning(
        #                             f"Failed to process table {table_idx} on page {page_num + 1}: {e}"
        #                         )
        #                         continue
                    ...
                    for table_idx, table in enumerate(camelot_tables):
                        if table.accuracy > 50:  # Only use high accuracy tables
                            try:
                                ...
                                logger.warning(
                                    f"Failed to process camelot table {table_idx}: {e}"
                                )
                                continue
    ...
    def _process_with_tabula(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            tabula_tables = tabula_py.read_pdf(
            ...
            for table_idx, df in enumerate(tabula_tables):
                if isinstance(df, pd.DataFrame) and not df.empty and len(df) > 1:
                    try:
                        ...
                        if not df.empty:
                            table_data = self._process_dataframe_table(
                                df, 0, table_idx, "tabula"
                            )
                            if table_data:
                                tables.append(table_data)
                    except Exception as e:
                        logger.warning(
                            f"Failed to process tabula table {table_idx}: {e}"
                        )
                        continue
    ...
    def _process_table_data(
        self, table: List, page_number: int, table_index: int, method: str
    ) -> Dict[str, Any]:
        ...
        for result_key in ["text", "ocr"]:
            if result_key in results and "text_chunks" in results[result_key]:
                ...
                if results[result_key]["text_chunks"]:
                    combined["metadata"]["processing_methods"].append(
                        result_key.title()
                    )
    ...
    def _deduplicate_tables(self, tables: List[Dict]) -> List[Dict]:
        ...
        logger.info(
            f"Deduplicated {len(tables)} tables to {len(unique_tables)} unique tables"
        )
